...
Transformer* qwen3_transformer_create(const char* checkpoint_path, int ctx_length) {
    Transformer* transformer = (Transformer*)calloc(1, sizeof(Transformer));
    if (!transformer) {
        fprintf(stderr, "Failed to allocate transformer\n");
        return NULL;
    }
    build_transformer(transformer, (char*)checkpoint_path, ctx_length);
    return transformer;
}
...
Tokenizer* qwen3_tokenizer_create(const char* checkpoint_path, int vocab_size, int enable_thinking) {
    ...
    build_tokenizer(tokenizer, (char*)checkpoint_path, vocab_size, enable_thinking);
    return tokenizer;
}
...
Sampler* qwen3_sampler_create(int vocab_size, float temperature, float topp, unsigned long long seed) {
    ...
    build_sampler(sampler, vocab_size, temperature, topp, seed);
    return sampler;
}
...
float* qwen3_forward(Transformer* transformer, int token, int position) {
    return forward(transformer, token, position);
}

int qwen3_sample_token(Sampler* sampler, float* logits) {
    return sample(sampler, logits);
}
...
void qwen3_sampler_set_seed(Sampler* sampler, unsigned long long seed) {
    if (!sampler) return;
    sampler->rng_state = seed ? seed : 1ull;
}
...
int qwen3_encode_text(Tokenizer* tokenizer, const char* text, int* tokens, int max_tokens) {
    ...
    int* temp = (int*)malloc(PROMPT_BUFFER_SIZE * sizeof(int));
    if (!temp) {
        fprintf(stderr, "Failed to allocate temporary encode buffer\n");
        return 0;
    }

    int n_tokens = 0;
    encode(tokenizer, (char*)text, temp, &n_tokens);

    int copy_count = n_tokens;
    if (copy_count > max_tokens) {
        copy_count = max_tokens;
    }

    memcpy(tokens, temp, copy_count * sizeof(int));
    free(temp);
    return copy_count;
}
...
int qwen3_get_vocab_size(const Transformer* transformer) {
    return transformer ? transformer->config.vocab_size : 0;
}

int qwen3_get_seq_len(const Transformer* transformer) {
    return transformer ? transformer->config.seq_len : 0;
}

int qwen3_get_dim(const Transformer* transformer) {
    return transformer ? transformer->config.dim : 0;
}

void qwen3_reset_kv_cache(Transformer* transformer) {
    if (!transformer) return;
    RunState* state = &transformer->state;
    size_t kv_elements = (size_t)transformer->config.n_layers * transformer->config.seq_len * transformer->config.n_kv_heads * transformer->config.head_dim;
    memset(state->key_cache, 0, kv_elements * sizeof(float));
    memset(state->value_cache, 0, kv_elements * sizeof(float));
    memset(state->x, 0, transformer->config.dim * sizeof(float));
    memset(state->xb, 0, transformer->config.n_heads * transformer->config.head_dim * sizeof(float));
    memset(state->hb, 0, transformer->config.hidden_dim * sizeof(float));
    memset(state->hb2, 0, transformer->config.hidden_dim * sizeof(float));
    memset(state->q, 0, transformer->config.n_heads * transformer->config.head_dim * sizeof(float));
    memset(state->att, 0, transformer->config.n_heads * transformer->config.seq_len * sizeof(float));
    memset(state->logits, 0, transformer->config.vocab_size * sizeof(float));
}
